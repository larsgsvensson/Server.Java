 ...
 * {@link ITriplePatternFragmentRequest}s over an index that provides an
 * overview of all available datasets.
 *
 * @author Miel Vander Sande
 * @author <a href="http://olafhartig.de">Olaf Hartig</a>
 */
public class IndexRequestProcessorForTPFs extends
	 ...
	 *            the base url
	 * @param datasources
	 *            a map of datasource with the datasource name as key and the
	 *            datasources as values
	 */
	public IndexRequestProcessorForTPFs(final String baseUrl,
			final HashMap<String, IDataSource> datasources) {
		this.model = ModelFactory.createDefaultModel();

		for (final Map.Entry<String, IDataSource> entry : datasources
				.entrySet()) {
			final String datasourceName = entry.getKey();
			final IDataSource datasource = entry.getValue();

			final Resource datasourceUrl = new ResourceImpl(baseUrl + "/"
					+ datasourceName);
	 ...
	 *
	 * @param request
	 *            the request the worker shall process
	 * @return a worker processing the specified request
	...
	protected class Worker
		 ...
		 *
		 * @param req
		 *            the requst this worker shall process
		 */
		public Worker(
		 ...
		 * @param limit
		 *            a limir
		 * @return a linked data fragment with the specified subject, predicate,
		 *         object, offset and limit
		 */
		@Override
		protected ILinkedDataFragment createFragment(
			...
			final Resource subject = s.isVariable() ? null : s.asConstantTerm()
					.asResource();
			final Property predicate = p.isVariable() ? null : ResourceFactory
					.createProperty(p.asConstantTerm().asResource().getURI());
			final RDFNode object = o.isVariable() ? null : o.asConstantTerm();

			final StmtIterator listStatements = IndexRequestProcessorForTPFs.this.model
			...
			return createTriplePatternFragment(result, result.size(),
					isLastPage);
		}
